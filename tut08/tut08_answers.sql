...
-- 1. Trigger to automatically increase salary by 10% for employees with salary below 60000
DELIMITER //
...
-- 2. Trigger to prevent deleting departments with assigned employees
DELIMITER //
...
-- 4. Trigger to assign a department to an employee based on salary range
DELIMITER //
...
-- 6. Trigger to prevent updating department_id for employees with assigned projects
DELIMITER //
...
-- 7. Trigger to update average salary for a department when salary changes occur
DELIMITER //
...
-- 8. Trigger to automatically delete records from works_on table for an employee when deleted from employees table
DELIMITER //
...
-- 9. Trigger to prevent inserting an employee with salary less than department's minimum salary
DELIMITER //
...
-- 10. Trigger to update total salary budget for a department when an employee's salary is updated
DELIMITER //
...
-- 11. Trigger to send email notification to HR when a new employee is hired
-- Create email queue table first
...
-- 12. Trigger to prevent inserting a new department if the location is not specified
DELIMITER //
...
-- 13. Trigger to update department_name in employees table when corresponding department_name is updated in departments table
DELIMITER //
...
-- 14. Trigger to log all insert, update, and delete operations on employees table into a separate audit table
...
-- 15. Trigger to automatically generate an employee ID using a sequence
DELIMITER //
